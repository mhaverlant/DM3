...
class Layer:
    ...
    def __init__(self, ninputs, noutputs, fa= 'sigmoid', lr =0.01, wdecay=0.0 , loss = 'none',dropout = False, dropout_rate = 0):
    ...
    def forward(self, inputs):
        ...
        if (self.dropout == True):
            ...
            #Pour tout neuronne on tire au hazard un chiffre entre 0 et 1 et on regarde si celui-ci est >dropout_rate sinon met a 0
            for i in range(0,self.ninputs):
                dropout_status = ran.randint(0,100)/100
                if dropout_status < self.dropout_rate:
                    self.statesIn[i]=0
...
class MLP:
    ...
    def __init__(self, SpecLayers, fa= 'sigmoid', lr= 0.01, wdecay=0.0 , n_iter = 10, dropout = False , dropout_rate = 0):
    ...
    def set_params(self, **parameters):
        ...
        #print n
        self.layers = []
        #on prend en compte le dropout des l'initialisation des couches
        for i in range(1,n):
            self.layers.append(Layer(self.SpecLayers[i-1],self.SpecLayers[i], fa = self.fa, wdecay = self.wdecay,dropout=self.dropout,dropout_rate=self.dropout_rate))
        self.set_lr(self.lr)
